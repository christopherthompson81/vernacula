...
## Run 34 — 2026-09-08 07:30 — the cache ceiling was a requirement, not a ceiling (issues #149, #150)

Two reports from the person who asked for this backend, on Windows with an RTX A4000 (16 GB)
and an A2000 alongside it: the 7B raises an allocation failure in `CreateSharedKvBuffers`, and
the 1.5B "seems to not use the GPU".

### The 7B on a 16 GB card

Reproduced on Linux by holding 6.6 GiB of VRAM in a second process so the run sees a 15.3 GiB
card, then transcribing 60 seconds with the published 7B package:

```
[ErrorCode:Fail] bfc_arena.cc:359 Failed to allocate memory for requested buffer of size 134217728
```

134,217,728 bytes is one KV tensor — `[1, 4, 131072, 128]` in float16, 128 MiB — and there are
56 of them. Measured cost of a 60-second run at the published ceilings:

| | weights | KV at the export ceiling | working set | measured peak |
|---|---|---|---|---|
| 1.5B (65,536) | 3.19 GiB | 1.75 GiB | 1.39 GiB | 6.33 GiB |
| 7B (131,072) | 9.03 GiB | 7.00 GiB | 2.36 GiB | 18.39 GiB |

The working set — ONNX Runtime's CUDA arena, the encoder's activations for one window, the
logits buffer — scales with the decoder's hidden size, which is what the reserve in the fix is
derived from.

So a one-minute file on the 7B paid 7.0 GiB for a cache sized to hold two hours. Nothing
needs that unless the recording actually is two hours long.

### Sizing the cache to the recording does not work on the published packages

First attempt: allocate `ceil(seconds × 16) + slack`, times a safety factor, clamped by free
VRAM. It fails immediately:

```
[ErrorCode:InvalidArgument] Got invalid dimensions for input: past_key_0
```

Run 20 raised the ceilings by rewriting the buffer length on the decoder's KV inputs, and that
length is a *fixed dimension*. ORT validates the bound tensor against it and rejects anything
else — so the exported number is not a ceiling the runtime may come in under, it is a
requirement the runtime must meet exactly. That is the whole of issue #150: the export decides
how much VRAM every run costs, and the 7B's decision does not fit on a 16 GB card.

`set_cache_ceiling.py --dynamic` replaces the fixed length with a symbolic one
(`kv_cache_len`) on all 112 KV inputs and outputs. GroupQueryAttention already takes the live
length through `seqlens_k` and `total_sequence_length` and reads the buffer stride from the
tensor it is given, so nothing else in the graph depends on the number. As with the ceiling
patch, only the ~0.9 MB graph file changes; `decoder_gqa.onnx.data` is byte-identical, so
republishing costs a megabyte rather than ten gigabytes.

Measured on the same 60-second file:

| | KV dim | peak | RTF | 16 GB card |
|---|---|---|---|---|
| 7B published | fixed 131,072 | 18.39 GiB | 0.197 | allocation fails |
| 7B dynamic | symbolic | 10.81 GiB | 0.196 | 11.32 GiB, RTF 0.192, completes |

Same transcript, same speed, 7.6 GiB less. A 16 GB card running the 7B now has about 4 GiB
...
covering speech denser than the measured average, since the buffer cannot grow mid-run),
clamps it to free VRAM less the working set, and refuses before any work with the length that
*would* fit when even the recording itself does not. A package with a fixed dimension still
gets exactly that dimension, so an older download keeps working unchanged.
...
The real gap is that nothing in the application answers the question. On a two-GPU machine
