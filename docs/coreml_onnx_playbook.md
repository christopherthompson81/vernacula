...
| Parakeet encoder | **Technique 6 works; the artifact is retired.** One partition, 2.5× per inference, bit-exact. But end to end in the app the ANE's win is ~5 s on a 10-minute file and opening the bucket sessions costs ~11.7 s, so it is a wash — see "the win has to survive the session" below. WebGPU then beat both on the stock dynamic graph (7.1 s vs the CPU EP's 10.8 s, no re-export at all), which retired it. Baking the length (the Sortformer recipe) was separately measured and rejected — 2–4% WER. |
