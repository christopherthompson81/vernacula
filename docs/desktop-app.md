...
Engine model locations, the default engine and its defaults live under **Settings → Text-to-Speech**. Each engine's ONNX export is a Hub repo the tab can download from — [`omnivoice-ipa-onnx`](https://huggingface.co/christopherthompson81/omnivoice-ipa-onnx), [`kokoro-82m-onnx`](https://huggingface.co/christopherthompson81/kokoro-82m-onnx), [`chatterbox-tts-onnx`](https://huggingface.co/christopherthompson81/chatterbox-tts-onnx) — built by `scripts/omnivoice_export`, `scripts/kokoro_export` and `scripts/chatterbox_export`. The OmniVoice voice library downloads from the same repo; the phonemizer data tree is found beside a source checkout. The tab reports what each engine is missing and where it looks. See [vernacula-tts](tts-cli.md) for what the OmniVoice engine needs.
