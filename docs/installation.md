...
- FFmpeg, both parts of it:
  ...
  - the **`ffmpeg` executable on `PATH`**, used by the CLI tools to decode anything that is not PCM/IEEE-float WAV or MP3 (FLAC, M4A, AAC, OGG, Opus, mu-law/A-law/ADPCM WAV)
  ...
  **WAV and MP3 need neither.** Both are decoded in-process, on every platform, so a machine with no FFmpeg at all still transcribes the two formats most recordings arrive in.
