...
public partial class HelpWindow : Window
{
    ...
    {
        // ThemeManager.Apply already called DwmHelper.SetDarkMode on all open windows.
    ...
    private Button MakeSidebarButton(HelpTopic topic)
    {
        var btn = new Button
        {
            ...
            Style               = (Style)Application.Current.Resources["SidebarTopicButtonStyle"],
        };
    ...
    public void DisplayTopic(string topicId)
    {
        ...
        try
        {
            ...
            // Add the panel to ContentViewer (which should be a Panel or similar container)
            if (ContentViewer is Panel panel)
            {
        ...
        {
            // Show error message in ContentViewer
            if (ContentViewer is Panel panel)
            {
                panel.Children.Clear();
    ...
    private void ContentViewer_PreviewMouseWheel(object sender, object e)
    {
        ...
        _contentScrollViewer.ScrollToVerticalOffset(
            _contentScrollViewer.VerticalOffset - e.Delta);
        e.Handled = true;
    }
