...
/// <summary>
/// Per-segment streaming event payload (shared by all backends). A chunk here is one
/// <see cref="Vernacula.Tts.Base.Markdown.TextSegment"/> — a markdown paragraph, heading, list
/// item or quote — rendered whole; an engine that has to split a long paragraph for its context
/// window joins the pieces before raising this.
/// </summary>
/// <param name="ChunkIndex">0-based segment index in document order.</param>
/// <param name="TotalChunks">Total segments the synthesis will produce.</param>
/// <param name="Audio24k">Raw float32 mono samples at the backend's sample rate.</param>
/// <param name="ChunkText">The segment's text (post-markdown-extraction).</param>
