...
internal partial class HomeViewModel : ObservableObject
{
    ...
    internal async Task CheckModelsAsync()
    {
        ...
        if (_settings.Current.Segmentation == Vernacula.Base.Models.SegmentationMode.DiariZen)
            await Task.Run(() => missingDiariZen = _modelMgr.GetMissingDiariZenFiles());

        ModelsReady   = missing.Count == 0 && (_settings.Current.Segmentation != Vernacula.Base.Models.SegmentationMode.DiariZen || missingDiariZen.Count == 0);
