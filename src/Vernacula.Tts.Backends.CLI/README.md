# Vernacula.Tts.Backends.CLI

One-shot command-line front end over
[`src/Vernacula.Tts.Base/`](../Vernacula.Tts.Base/). Synthesizes a single
...
```bash
# Inline text:
dotnet run --project src/Vernacula.Tts.Backends.CLI -- \
...
Once `dotnet publish -c Release` is run, the binary is named `vernacula-tts-backends`
(see the `AssemblyName` in `Vernacula.Tts.Backends.CLI.csproj`).
