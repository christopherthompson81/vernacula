...
public sealed class OmniVoice : IDisposable
{
    ...
    // NOTE: an OrtIoBinding-based TransformerLoop was tried here to reuse pinned buffers across
    // diffusion steps, but binding CPU-pinned OrtValues to a CUDA session produced corrupt output
    // (audible noise) for a ~90 ms/gen saving. Removed — RunDiffusion uses the plain RunTransformer
    // above. See docs/omnivoice_onnx_investigation.md.
